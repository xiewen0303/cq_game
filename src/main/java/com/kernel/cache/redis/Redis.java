...
public class Redis {
	...
	public Redis(GenericObjectPoolConfig config, String host, int port, int timeout, String password, int database) {
		...
		ping();
		ChuanQiLog.info("Redis 连接信息:host={},port={},database={},timeout={}ms,password={}", new Object[] { host, port, database, timeout, password });
	}
	...
	public boolean ping() {
		...
		try {
			...
			if (ret.equals("PONG")){
				ChuanQiLog.info("Redis服务器连接正常");
				return true;
			}
			else{
				ChuanQiLog.error("Redis服务器连接异常");
				return false;
			}
		} catch (Exception e) {
	...
	public static void migrate(String fromHost, int fromPort, int fromIndex, String toHost, int toPort, int toIndex, int timeout, String... list) {
		...
		ChuanQiLog.info("数据迁移开始 fromHost={},fromPort={},fromIndex={},toHost={},toPort={},toIndex={},timeout={},list={}", fromHost, fromPort, fromIndex, toHost,
		...
		redis.destory();
		ChuanQiLog.info("数据迁移完成 inerval={},ret={}", (System.nanoTime() - start) / 1000000f, ret);
	}
