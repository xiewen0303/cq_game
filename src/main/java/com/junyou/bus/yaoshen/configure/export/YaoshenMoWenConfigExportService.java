...
@Component
public class YaoshenMoWenConfigExportService extends
	 ...
	 * configFileName
	 */
	private String configureName = "YaoShenMoWen.jat";

	private Map<Integer, Map<Integer, YaoshenJichuConfig>> configMap;
	private Map<Integer, Map<String, Long>> attrMap=new HashMap<Integer, Map<String, Long>>();

	private YaoshenJichuConfig createChengHaoPeiZhiConfig(
			Map<String, Object> tmp) {
		YaoshenJichuConfig config = new YaoshenJichuConfig();
	...
	protected void configureDataResolve(byte[] data) {
		...
		Map<Integer, Map<Integer, YaoshenJichuConfig>> configMap = new HashMap<Integer, Map<Integer, YaoshenJichuConfig>>();
		List<Integer> idList = new ArrayList<Integer>();
		for (Object obj : dataList) {
			Map<String, Object> tmp = (Map<String, Object>) obj;
			if (null != tmp) {
				YaoshenJichuConfig config = createChengHaoPeiZhiConfig(tmp);
				...
				Map<Integer, YaoshenJichuConfig> map = configMap.get(config
						.getId1());
				if (map == null) {
					map = new HashMap<Integer, YaoshenJichuConfig>();
...
//		attrMap = tmpAttrMap;
	...
	public YaoshenJichuConfig load(Integer jie, Integer ceng) {
		Map<Integer, YaoshenJichuConfig> map = configMap.get(jie);
		if (map == null) {
			return null;
		}
		return map.get(ceng);
	}
	public Map<Integer, YaoshenJichuConfig> loadByJie(Integer jie) {
		Map<Integer, YaoshenJichuConfig> map = configMap.get(jie);
		if (map == null) {
			return null;
		}
	...
	public Map<String, Long> getYaoshenAttribute(int jie, int ceng) {
		YaoshenJichuConfig config = load(jie, ceng);
