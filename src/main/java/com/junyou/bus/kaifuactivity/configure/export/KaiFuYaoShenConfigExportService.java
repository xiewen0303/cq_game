 ...
 */
@Service
public class KaiFuYaoShenConfigExportService  extends AbstractRfbConfigService {
	@Override
	public Object getChildData(int subId) {
		KaiFuYaoShenGroupConfig subCfg = KFPM_MAP.get(subId);
		return subCfg != null ? subCfg.getPic() : null;
	}
	
	private static final KaiFuYaoShenConfigExportService INSTANCE = new KaiFuYaoShenConfigExportService();

	/**
	 * key:子活动id
	 * value:子活动数据
	 */
	private Map<Integer,KaiFuYaoShenGroupConfig> KFPM_MAP = new HashMap<>();
	
	
	private KaiFuYaoShenConfigExportService(){
	}
	 ...
	 * @return
	 */
	public static KaiFuYaoShenConfigExportService getInstance() {
		return INSTANCE;
	}
	
	public Map<Integer,KaiFuYaoShenGroupConfig> getAllConfig(){
		return KFPM_MAP;
	}
	
	public KaiFuYaoShenConfig loadByKeyId(int subId,Integer id){
		KaiFuYaoShenGroupConfig config = KFPM_MAP.get(subId);
		if(config != null){
	...
	public KaiFuYaoShenGroupConfig loadByMap(int subId){
	...
	public void analysisConfigureDataResolve(int subId,byte[] data) {
		...
		KaiFuYaoShenGroupConfig group = KFPM_MAP.get(subId);
		if(group != null){
		...
		group = new KaiFuYaoShenGroupConfig();
		...
		Map<Integer, KaiFuYaoShenConfig> tmpConfig = new HashMap<Integer, KaiFuYaoShenConfig>();
		for (Map<String,Object> map : list) {
			if (null != map) {
				...
				if(id == -1){
					...
					KaiFuYaoShenConfig config = createKaiFuYaoShenConfig(map);
	...
	private void setData(Map<Integer, KaiFuYaoShenConfig> configMap,KaiFuYaoShenGroupConfig group){
	...
	public KaiFuYaoShenConfig createKaiFuYaoShenConfig(Map<String, Object> tmp) {
		KaiFuYaoShenConfig config = new KaiFuYaoShenConfig();	
