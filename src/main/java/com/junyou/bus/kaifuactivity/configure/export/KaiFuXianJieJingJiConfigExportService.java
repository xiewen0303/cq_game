 ...
 */
@Service
public class KaiFuXianJieJingJiConfigExportService  extends AbstractRfbConfigService {

	private static final KaiFuXianJieJingJiConfigExportService INSTANCE = new KaiFuXianJieJingJiConfigExportService();

	/**
	 * key:子活动id
	 * value:子活动数据
	 */
	private Map<Integer,KaiFuXianJieJingJiGroupConfig> KFPM_MAP = new HashMap<>();
	
	
	private KaiFuXianJieJingJiConfigExportService(){
	}
	 ...
	 * @return
	 */
	public static KaiFuXianJieJingJiConfigExportService getInstance() {
		return INSTANCE;
	}
	
	public Map<Integer,KaiFuXianJieJingJiGroupConfig> getAllConfig(){
		return KFPM_MAP;
	}
	
	public KaiFuXianJieJingJiConfig loadByKeyId(int subId,Integer id){
		KaiFuXianJieJingJiGroupConfig config = KFPM_MAP.get(subId);
		if(config != null){
	...
	public KaiFuXianJieJingJiGroupConfig loadByMap(int subId){
	...
	public void analysisConfigureDataResolve(int subId,byte[] data) {
		if(data == null){
			ChuanQiLog.error(" KaiFuXianJieJingJi 1 data is error! ");
		}
		...
		if(json == null || json.isEmpty()){
			ChuanQiLog.error(" KaiFuXianJieJingJi 2 data is error! ");
			return;
		}
		
		//版本号比对
		String md5Value = Md5Utils.md5Bytes(data);
		KaiFuXianJieJingJiGroupConfig group = KFPM_MAP.get(subId);
		if(group != null){
			
			if(md5Value.equals(group.getMd5Version())){
				//版本号一致，不处理下面的业务直接跳出
				ChuanQiLog.error(" KaiFuXianJieJingJi subid={} version is same md5Value={}",subId,md5Value);
				return;
			}
		}
		group = new KaiFuXianJieJingJiGroupConfig();
		...
		Map<Integer, KaiFuXianJieJingJiConfig> tmpConfig = new HashMap<Integer, KaiFuXianJieJingJiConfig>();
		for (Map<String,Object> map : list) {
			if (null != map) {
				...
				if(id == -1){
					...
					KaiFuXianJieJingJiConfig config = createKaiFuXianJieJingJiConfig(map);
	...
	private void setData(Map<Integer, KaiFuXianJieJingJiConfig> configMap,KaiFuXianJieJingJiGroupConfig group){
	...
	public KaiFuXianJieJingJiConfig createKaiFuXianJieJingJiConfig(Map<String, Object> tmp) {
		KaiFuXianJieJingJiConfig config = new KaiFuXianJieJingJiConfig();	
	...
	public Object getChildData(int subId) {
		KaiFuXianJieJingJiGroupConfig  config = this.KFPM_MAP.get(subId);
