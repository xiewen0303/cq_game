 ...
 */
@Service
public class KaiFuChiBangConfigExportService extends AbstractRfbConfigService {

	private static final KaiFuChiBangConfigExportService INSTANCE = new KaiFuChiBangConfigExportService();

	/**
	 * key:子活动id
	 * value:子活动数据
	 */
	private Map<Integer,KaiFuChiBangGroupConfig> KFPM_MAP = new HashMap<>();
	
	
	private KaiFuChiBangConfigExportService(){
	}
	 ...
	 * @return
	 */
	public static KaiFuChiBangConfigExportService getInstance() {
		return INSTANCE;
	}
	
	public Map<Integer,KaiFuChiBangGroupConfig> getAllConfig(){
		return KFPM_MAP;
	}
	
	public KaiFuChiBangConfig loadByKeyId(int subId,Integer id){
		KaiFuChiBangGroupConfig config = KFPM_MAP.get(subId);
		if(config != null){
	...
	public KaiFuChiBangGroupConfig loadByMap(int subId){
	...
	public void analysisConfigureDataResolve(int subId,byte[] data) {
		...
		KaiFuChiBangGroupConfig group = KFPM_MAP.get(subId);
		if(group != null){
		...
		group = new KaiFuChiBangGroupConfig();
		...
		Map<Integer, KaiFuChiBangConfig> tmpConfig = new HashMap<Integer, KaiFuChiBangConfig>();
		for (Map<String,Object> map : list) {
			if (null != map) {
				...
				if(id == -1){
					...
					KaiFuChiBangConfig config = createKaiFuChiBangConfig(map);
	...
	private void setData(Map<Integer, KaiFuChiBangConfig> configMap,KaiFuChiBangGroupConfig group){
	...
	public KaiFuChiBangConfig createKaiFuChiBangConfig(Map<String, Object> tmp) {
		KaiFuChiBangConfig config = new KaiFuChiBangConfig();	
	...
	public Object getChildData(int subId) {
		KaiFuChiBangGroupConfig config = this.KFPM_MAP.get(subId);
