 ...
 */
@Service
public class KaiFuTangBaoConfigExportService extends AbstractRfbConfigService {

	@Override
	public Object getChildData(int subId) {
		KaiFuTangBaoGroupConfig cofnig = this.KFPM_MAP.get(subId);
		return cofnig != null ? cofnig.getPic() : null;
	}
	
	private static final KaiFuTangBaoConfigExportService INSTANCE = new KaiFuTangBaoConfigExportService();

	/**
	 * key:子活动id
	 * value:子活动数据
	 */
	private Map<Integer,KaiFuTangBaoGroupConfig> KFPM_MAP = new HashMap<>();
	
	
	private KaiFuTangBaoConfigExportService(){
	}
	 ...
	 * @return
	 */
	public static KaiFuTangBaoConfigExportService getInstance() {
		return INSTANCE;
	}
	
	public Map<Integer,KaiFuTangBaoGroupConfig> getAllConfig(){
		return KFPM_MAP;
	}
	
	public KaiFuTangBaoConfig loadByKeyId(int subId,Integer id){
		KaiFuTangBaoGroupConfig config = KFPM_MAP.get(subId);
		if(config != null){
	...
	public KaiFuTangBaoGroupConfig loadByMap(int subId){
	...
	public void analysisConfigureDataResolve(int subId,byte[] data) {
		if(data == null){
			ChuanQiLog.error(" KaiFuTangBao 1 data is error! ");
		}
		...
		if(json == null || json.isEmpty()){
			ChuanQiLog.error(" KaiFuTangBao 2 data is error! ");
			return;
		}
		
		//版本号比对
		String md5Value = Md5Utils.md5Bytes(data);
		KaiFuTangBaoGroupConfig group = KFPM_MAP.get(subId);
		if(group != null){
			
			if(md5Value.equals(group.getMd5Version())){
				//版本号一致，不处理下面的业务直接跳出
				ChuanQiLog.error(" KaiFuTangBao subid={} version is same md5Value={}",subId,md5Value);
				return;
			}
		}
		group = new KaiFuTangBaoGroupConfig();
		...
		Map<Integer, KaiFuTangBaoConfig> tmpConfig = new HashMap<Integer, KaiFuTangBaoConfig>();
		for (Map<String,Object> map : list) {
			if (null != map) {
				...
				if(id == -1){
					...
					KaiFuTangBaoConfig config = createKaiFuTangBaoConfig(map);
	...
	private void setData(Map<Integer, KaiFuTangBaoConfig> configMap,KaiFuTangBaoGroupConfig group){
	...
	public KaiFuTangBaoConfig createKaiFuTangBaoConfig(Map<String, Object> tmp) {
		KaiFuTangBaoConfig config = new KaiFuTangBaoConfig();	
		
		config.setId(CovertObjectUtil.object2int(tmp.get("id")));
		
		config.setMax(CovertObjectUtil.object2int(tmp.get("max")));
		config.setMin(CovertObjectUtil.object2int(tmp.get("min")));
		config.setTangBaoLevel(CovertObjectUtil.object2int(tmp.get("jieduan")));
