 ...
 */
@Service
public class KaiFuYaoMoConfigExportService extends AbstractRfbConfigService {
	@Override
	public Object getChildData(int subId) {
		KaiFuYaoMoGroupConfig subCfg = KFPM_MAP.get(subId);
		return subCfg != null ? subCfg.getPic() : null;
	}
	private static final KaiFuYaoMoConfigExportService INSTANCE = new KaiFuYaoMoConfigExportService();

	/**
	 * key:子活动id
	 * value:子活动数据
	 */
	private Map<Integer,KaiFuYaoMoGroupConfig> KFPM_MAP = new HashMap<>();
	
	
	private KaiFuYaoMoConfigExportService(){
	}
	 ...
	 * @return
	 */
	public static KaiFuYaoMoConfigExportService getInstance() {
		return INSTANCE;
	}
	
	public Map<Integer,KaiFuYaoMoGroupConfig> getAllConfig(){
		return KFPM_MAP;
	}
	
	public KaiFuYaoMoConfig loadByKeyId(int subId,Integer id){
		KaiFuYaoMoGroupConfig config = KFPM_MAP.get(subId);
		if(config != null){
	...
	public KaiFuYaoMoGroupConfig loadByMap(int subId){
	...
	public void analysisConfigureDataResolve(int subId,byte[] data) {
		...
		KaiFuYaoMoGroupConfig group = KFPM_MAP.get(subId);
		if(group != null){
		...
		group = new KaiFuYaoMoGroupConfig();
		...
		Map<Integer, KaiFuYaoMoConfig> tmpConfig = new HashMap<Integer, KaiFuYaoMoConfig>();
		for (Map<String,Object> map : list) {
			if (null != map) {
				...
				if(id == -1){
					...
					KaiFuYaoMoConfig config = createKaiFuYaoMoConfig(map);
	...
	private void setData(Map<Integer, KaiFuYaoMoConfig> configMap,KaiFuYaoMoGroupConfig group){
	...
	public KaiFuYaoMoConfig createKaiFuYaoMoConfig(Map<String, Object> tmp) {
		KaiFuYaoMoConfig config = new KaiFuYaoMoConfig();	
		
		config.setId(CovertObjectUtil.object2int(tmp.get("id")));
		
		config.setMax(CovertObjectUtil.object2int(tmp.get("max")));
		config.setMin(CovertObjectUtil.object2int(tmp.get("min")));
		config.setyaomoLevel(CovertObjectUtil.object2int(tmp.get("jieduan")));
