 ...
 */
@Service
public class KaiFuQiangHuaConfigExportService extends AbstractRfbConfigService{

	@Override
	public Object getChildData(int subId) {
		KaiFuQiangHuaGroupConfig config = KFPM_MAP.get(subId);
		return config != null ? config.getPic() : null;
	}


	private static final KaiFuQiangHuaConfigExportService INSTANCE = new KaiFuQiangHuaConfigExportService();

	/**
	 * key:子活动id
	 * value:子活动数据
	 */
	private Map<Integer,KaiFuQiangHuaGroupConfig> KFPM_MAP = new HashMap<>();
	
	
	private KaiFuQiangHuaConfigExportService(){
	}
	 ...
	 * @return
	 */
	public static KaiFuQiangHuaConfigExportService getInstance() {
		return INSTANCE;
	}
	
	public Map<Integer,KaiFuQiangHuaGroupConfig> getAllConfig(){
		return KFPM_MAP;
	}
	
	public KaiFuQiangHuaConfig loadByKeyId(int subId,Integer id){
		KaiFuQiangHuaGroupConfig config = KFPM_MAP.get(subId);
		if(config != null){
	...
	public KaiFuQiangHuaGroupConfig loadByMap(int subId){
	...
	public void analysisConfigureDataResolve(int subId,byte[] data) {
		if(data == null){
			ChuanQiLog.error(" KaiFuQiangHua 1 data is error! ");
		}
		...
		if(json == null || json.isEmpty()){
			ChuanQiLog.error(" KaiFuQiangHua 2 data is error! ");
			return;
		}
		
		//版本号比对
		String md5Value = Md5Utils.md5Bytes(data);
		KaiFuQiangHuaGroupConfig group = KFPM_MAP.get(subId);
		if(group != null){
			
			if(md5Value.equals(group.getMd5Version())){
				//版本号一致，不处理下面的业务直接跳出
				ChuanQiLog.error(" KaiFuQiangHua subid={} version is same md5Value={}",subId,md5Value);
				return;
			}
		}
		group = new KaiFuQiangHuaGroupConfig();
		...
		Map<Integer, KaiFuQiangHuaConfig> tmpConfig = new HashMap<Integer, KaiFuQiangHuaConfig>();
		for (Map<String,Object> map : list) {
			if (null != map) {
				...
				if(id == -1){
					...
					KaiFuQiangHuaConfig config = createKaiFuQiangHuaConfig(map);
	...
	private void setData(Map<Integer, KaiFuQiangHuaConfig> configMap,KaiFuQiangHuaGroupConfig group){
	...
	public KaiFuQiangHuaConfig createKaiFuQiangHuaConfig(Map<String, Object> tmp) {
		KaiFuQiangHuaConfig config = new KaiFuQiangHuaConfig();	
		
		config.setId(CovertObjectUtil.object2int(tmp.get("id")));
		
		config.setMax(CovertObjectUtil.object2int(tmp.get("max")));
		config.setMin(CovertObjectUtil.object2int(tmp.get("min")));
		config.setQiangHuaLevel(CovertObjectUtil.object2int(tmp.get("jieduan")));
