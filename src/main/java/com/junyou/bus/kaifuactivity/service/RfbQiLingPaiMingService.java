...
import com.junyou.bus.kaifuactivity.configure.export.KaiFuQiLingConfig;
import com.junyou.bus.kaifuactivity.configure.export.KaiFuQiLingConfigExportService;
import com.junyou.bus.kaifuactivity.configure.export.KaiFuQiLingGroupConfig;
 ...
 */
@Service
public class RfbQiLingPaiMingService { 
	...
	/**器灵排行相关 **/
	public void start(List<IQiLingRankVo> list) {
		Map<Integer,KaiFuQiLingGroupConfig> vo = KaiFuQiLingConfigExportService.getInstance().getAllConfig();
	...
	public void zhanLiPaiXing(List<IQiLingRankVo> userList,int subId) {
		//判断是否已经发过邮件
		...
		KaiFuQiLingGroupConfig config = KaiFuQiLingConfigExportService.getInstance().loadByMap(subId);
		if(config == null){
			return;
		}
		//处理数据
		Map<Integer, KaiFuQiLingConfig> configMap = config.getConfigMap();
		...
		try {
			...
			GamePublishEvent.publishEvent(new PaiMingLogEvent(LogPrintHandle.PAIMING_QILING_PAIMING,consumeItemArray));
		} catch (Exception e) {
			ChuanQiLog.error(""+e);
		}
		...
		for (int i = 21; i <= 40; i++) {
			...
			for (int j = start; j <= end; j++) {
				IQiLingRankVo vo =  userList.get(j-1);
				long userRoleId = vo.getUserRoleId();
				String title = EmailUtil.getCodeEmail(AppErrorCode.KAIFU_PAIMING_EMAIL_TITLE);
				String content = EmailUtil.getCodeEmail(AppErrorCode.KAIFU_QILING_PAIMING,j+"");
				emailExportService.sendEmailToOne(userRoleId,title, content,GameConstants.EMAIL_TYPE_SINGLE, zhanliConfig.getEmailItem());
			
				//日志
				GamePublishEvent.publishEvent(new KaiFuPaiMingHDLogEvent(userRoleId, vo.getName(), j, zhanliConfig.getEmailItem(), LogPrintHandle.QILING_PAIMING));
			}
	 ...
	 * @param receiveItems
	 */
	public static void parseJSONArray(List<IQiLingRankVo> userList,JSONArray receiveItems) {
		if(userList == null || userList.size() == 0){
			return;
		}
		for (int i = 0; i < userList.size();i++) {
			IQiLingRankVo vo =  userList.get(i);
	...
	public Object[] lingQuJiangLi(long userRoleId,Integer version, Integer configId,int subId) {
		...
		//判断配置
		KaiFuQiLingConfig config = KaiFuQiLingConfigExportService.getInstance().loadByKeyId(subId,configId);
		if(config== null){
			return AppErrorCode.CONFIG_ERROR;
		}
		//玩家器灵
		Integer qilingLevel = qiLingExportService.getQiLingLevel(userRoleId);
		if(qilingLevel == null){
			return AppErrorCode.KAIFU_TIAOJIAN_ERROR;
		}
		QiLingJiChuConfig qiLingJiChuConfig = qiLingJiChuConfigExportService.loadById(qilingLevel);
		if(qiLingJiChuConfig == null || qiLingJiChuConfig.getLevel() < config.getQiLingLevel()){
			return AppErrorCode.KAIFU_TIAOJIAN_ERROR;
		}
		...
		//发放奖励
		roleBagExportService.putGoodsVoAndNumberAttr(config.getJiangitem(), userRoleId, GoodsSource.GOODS_GET_CBPM, LogPrintHandle.GET_RFB_QILINGPAIMING, LogPrintHandle.GBZ_RFB_QILING, true);
		...
		GamePublishEvent.publishEvent(new KaiFuPaiMingLingQuLogEvent(userRoleId, loginRole.getName(), receiveItems, LogPrintHandle.KAIFU_ACTITY_LQ_QL));
	...
	public Object[] getRefbInfo(Long userRoleId, Integer subId){
		...
		Object[] one = rankExportService.getKuiShouInfoByType(GameConstants.QILING_TYPE);
		int myMingci = rankExportService.getMyOrderByType(userRoleId, GameConstants.QILING_TYPE);
