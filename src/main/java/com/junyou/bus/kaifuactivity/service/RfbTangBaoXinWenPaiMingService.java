...
import com.junyou.bus.kaifuactivity.configure.export.KaiFuTangBaoXinWenConfig;
import com.junyou.bus.kaifuactivity.configure.export.KaiFuTangBaoXinWenConfigExportService;
import com.junyou.bus.kaifuactivity.configure.export.KaiFuTangBaoXinWenGroupConfig;
 ...
 */
@Service
public class RfbTangBaoXinWenPaiMingService extends AbstractActivityService {

	@Override
	public boolean getChildFlag(long userRoleId,int subId) {
		
		KaiFuTangBaoXinWenGroupConfig configs = KaiFuTangBaoXinWenConfigExportService.getInstance().loadByMap(subId);
		if(configs == null){
			return false;
		}
		
		//处理数据
		Map<Integer, KaiFuTangBaoXinWenConfig> configMap = configs.getConfigMap();
		
		//如果这两个同时存在,则不可领取奖励
		KaiFuTangBaoXinWenConfig zhanliConfig =  configMap.get(1);//ID为1规定为阶段邮件奖励
		KaiFuTangBaoXinWenConfig paihengConfig =  configMap.get(21);//判断是否有排行配置
		if(paihengConfig != null && zhanliConfig != null){
			return false;
		}
		
		for (KaiFuTangBaoXinWenConfig config : configMap.values()) {
			if(config.getId() < 0){
				continue;
			}
			//玩家翅膀
			int xinWenLevel = xinwenExportService.getTangbaoXinwenJie(userRoleId);
			if(xinWenLevel < config.getTangBaoXinWenLevel()){
				continue;
			}
	...
	/**翅膀排行相关 **/
	public void start(List<IXinwenRankVo> list) {
		Map<Integer,KaiFuTangBaoXinWenGroupConfig> vo = KaiFuTangBaoXinWenConfigExportService.getInstance().getAllConfig();
	...
	public void zhanLiPaiXing(List<IXinwenRankVo> userList,int subId) {
		//判断是否已经发过邮件
		...
		KaiFuTangBaoXinWenGroupConfig config = KaiFuTangBaoXinWenConfigExportService.getInstance().loadByMap(subId);
		if(config == null){
			return;
		}
		//处理数据
		Map<Integer, KaiFuTangBaoXinWenConfig> configMap = config.getConfigMap();
		...
		try {
			...
			GamePublishEvent.publishEvent(new PaiMingLogEvent(LogPrintHandle.PAIMING_TANG_BAO_XINWEN,consumeItemArray));
		} catch (Exception e) {
			ChuanQiLog.error(""+e);
		}
		...
		for (int i = 21; i <= 40; i++) {
			...
			for (int j = start; j <= end; j++) {
				IXinwenRankVo vo =  userList.get(j-1);
				long userRoleId = vo.getUserRoleId();
				String title = EmailUtil.getCodeEmail(AppErrorCode.KAIFU_PAIMING_EMAIL_TITLE);
				String content = EmailUtil.getCodeEmail(AppErrorCode.KAIFU_TANGBAO_XINWEN_PAIMING,j+"");
				emailExportService.sendEmailToOne(userRoleId,title, content,GameConstants.EMAIL_TYPE_SINGLE, zhanliConfig.getEmailItem());
			
				//日志
				GamePublishEvent.publishEvent(new KaiFuPaiMingHDLogEvent(userRoleId, vo.getName(), j, zhanliConfig.getEmailItem(), LogPrintHandle.TANGBAO_XINWEN));
			}
	 ...
	 * @param receiveItems
	 */
	public static void parseJSONArray(List<IXinwenRankVo> userList,JSONArray receiveItems) {
		if(userList == null || userList.size() == 0){
			return;
		}
		for (int i = 0; i < userList.size();i++) {
			IXinwenRankVo vo =  userList.get(i);
	...
	public Object[] lingQuJiangLi(long userRoleId,Integer version, Integer configId,int subId) {
		...
		//判断配置
		KaiFuTangBaoXinWenConfig config = KaiFuTangBaoXinWenConfigExportService.getInstance().loadByKeyId(subId,configId);
		if(config== null){
		...
		//发放奖励
		roleBagExportService.putGoodsVoAndNumberAttr(config.getJiangitem(), userRoleId, GoodsSource.GOODS_GET_TBXWPM, LogPrintHandle.GET_REFABU_TANGBAOXINWEN, LogPrintHandle.GBZ_REFABU_TANGBAOXINWEN, true);
		...
		GamePublishEvent.publishEvent(new KaiFuPaiMingLingQuLogEvent(userRoleId, loginRole.getName(), receiveItems, LogPrintHandle.KAIFU_ACTITY_LQ_TBXW));
	...
	public Object[] getRefbInfo(Long userRoleId, Integer subId){
		...
		Object[] one = rankExportService.getKuiShouInfoByType(GameConstants.TANGBAO_XINWEN_TYPE);
		int myMingci = rankExportService.getMyOrderByType(userRoleId, GameConstants.TANGBAO_XINWEN_TYPE);
