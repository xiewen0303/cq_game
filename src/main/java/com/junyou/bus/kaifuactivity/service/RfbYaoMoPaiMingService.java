...
import com.junyou.bus.kaifuactivity.configure.export.KaiFuYaoMoConfig;
import com.junyou.bus.kaifuactivity.configure.export.KaiFuYaoMoConfigExportService;
import com.junyou.bus.kaifuactivity.configure.export.KaiFuYaoMoGroupConfig;
 ...
 */
@Service
public class RfbYaoMoPaiMingService { 
	...
	@Autowired
	private YaoshenExportService yaoshenExportServic;
	
	/**翅膀排行相关 **/
	public void start(List<IYaoShenMowenRankVo> list) {
		Map<Integer,KaiFuYaoMoGroupConfig> vo = KaiFuYaoMoConfigExportService.getInstance().getAllConfig();
	...
	public void zhanLiPaiXing(List<IYaoShenMowenRankVo> userList,int subId) {
		//判断是否已经发过邮件
		...
		KaiFuYaoMoGroupConfig config = KaiFuYaoMoConfigExportService.getInstance().loadByMap(subId);
		if(config == null){
			return;
		}
		//处理数据
		Map<Integer, KaiFuYaoMoConfig> configMap = config.getConfigMap();
		...
		try {
			...
			GamePublishEvent.publishEvent(new PaiMingLogEvent(LogPrintHandle.PAIMING_YAOSHEN_MOWEN_PAIMING,consumeItemArray));
		} catch (Exception e) {
			ChuanQiLog.error(""+e);
		}
		
		for (int i = 21; i <= 40; i++) {
			KaiFuYaoMoConfig zhanliConfig =  configMap.get(i);
			...
			for (int j = start; j <= end; j++) {
				IYaoShenMowenRankVo vo =  userList.get(j-1);
				long userRoleId = vo.getUserRoleId();
				String title = EmailUtil.getCodeEmail(AppErrorCode.KAIFU_PAIMING_EMAIL_TITLE);
				String content = EmailUtil.getCodeEmail(AppErrorCode.KAIFU_YAOSHEN_MOWEN_PAIMING,j+"");
				emailExportService.sendEmailToOne(userRoleId,title, content,GameConstants.EMAIL_TYPE_SINGLE, zhanliConfig.getEmailItem());
			
				//日志
				GamePublishEvent.publishEvent(new KaiFuPaiMingHDLogEvent(userRoleId, vo.getName(), j, zhanliConfig.getEmailItem(), LogPrintHandle.YAOSHEN_MOWEN));
			}
	 ...
	 * @param receiveItems
	 */
	public static void parseJSONArray(List<IYaoShenMowenRankVo> userList,JSONArray receiveItems) {
		if(userList == null || userList.size() == 0){
			return;
		}
		for (int i = 0; i < userList.size();i++) {
			IYaoShenMowenRankVo vo =  userList.get(i);
	...
	public Object[] lingQuJiangLi(long userRoleId,Integer version, Integer configId,int subId) {
		...
		//判断配置
		KaiFuYaoMoConfig config = KaiFuYaoMoConfigExportService.getInstance().loadByKeyId(subId,configId);
		if(config== null){
			return AppErrorCode.CONFIG_ERROR;
		}
		//玩家魔纹
		RoleYaoshenMowen yaoshen = yaoshenExportServic.getRoleYaoshenMowen(userRoleId);
		if(yaoshen == null || yaoshen.getJieLevel() < config.getYaomoLevel()){
		...
		//发放奖励
		roleBagExportService.putGoodsVoAndNumberAttr(config.getJiangitem(), userRoleId, GoodsSource.GOODS_GET_YSMW, LogPrintHandle.GET_RFB_YAOSHENMOWEN, LogPrintHandle.GBZ_RFB_YAOSHENMOWEN, true);
		...
		GamePublishEvent.publishEvent(new KaiFuPaiMingLingQuLogEvent(userRoleId, loginRole.getName(), receiveItems, LogPrintHandle.KAIFU_ACTITY_LQ_YSMW));
	...
	public Object[] getRefbInfo(Long userRoleId, Integer subId){
		...
		Object[] one = rankExportService.getKuiShouInfoByType(GameConstants.YS_MOWEN_TYPE);
		int myMingci = rankExportService.getMyOrderByType(userRoleId, GameConstants.YS_MOWEN_TYPE);
