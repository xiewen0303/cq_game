...
import com.junyou.bus.kaifuactivity.configure.export.KaiFuYaoShenConfig;
import com.junyou.bus.kaifuactivity.configure.export.KaiFuYaoShenConfigExportService;
import com.junyou.bus.kaifuactivity.configure.export.KaiFuYaoShenGroupConfig;
 ...
 */
@Service
public class RfbYaoShenPaiMingService { 
	...
	/**翅膀排行相关 **/
	public void start(List<IYaoShenRankVo> list) {
		Map<Integer,KaiFuYaoShenGroupConfig> vo = KaiFuYaoShenConfigExportService.getInstance().getAllConfig();
	...
	public void zhanLiPaiXing(List<IYaoShenRankVo> userList,int subId) {
		//判断是否已经发过邮件
		...
		KaiFuYaoShenGroupConfig config = KaiFuYaoShenConfigExportService.getInstance().loadByMap(subId);
		if(config == null){
			return;
		}
		//处理数据
		Map<Integer, KaiFuYaoShenConfig> configMap = config.getConfigMap();
		...
		try {
			...
			GamePublishEvent.publishEvent(new PaiMingLogEvent(LogPrintHandle.PAIMING_YAOSHEN_PAIMING,consumeItemArray));
		} catch (Exception e) {
			ChuanQiLog.error(""+e);
		}
		
		for (int i = 21; i <= 40; i++) {
			KaiFuYaoShenConfig zhanliConfig =  configMap.get(i);
			...
			for (int j = start; j <= end; j++) {
				IYaoShenRankVo vo =  userList.get(j-1);
				long userRoleId = vo.getUserRoleId();
				String title = EmailUtil.getCodeEmail(AppErrorCode.KAIFU_PAIMING_EMAIL_TITLE);
				String content = EmailUtil.getCodeEmail(AppErrorCode.KAIFU_YAOSHEN_PAIMING,j+"");
				emailExportService.sendEmailToOne(userRoleId,title, content,GameConstants.EMAIL_TYPE_SINGLE, zhanliConfig.getEmailItem());
			
				//日志
				GamePublishEvent.publishEvent(new KaiFuPaiMingHDLogEvent(userRoleId, vo.getName(), j, zhanliConfig.getEmailItem(), LogPrintHandle.YAOSHEN_PAIMING));
			}
	 ...
	 * @param receiveItems
	 */
	public static void parseJSONArray(List<IYaoShenRankVo> userList,JSONArray receiveItems) {
		if(userList == null || userList.size() == 0){
			return;
		}
		for (int i = 0; i < userList.size();i++) {
			IYaoShenRankVo vo =  userList.get(i);
	...
	public Object[] lingQuJiangLi(long userRoleId,Integer version, Integer configId,int subId) {
		...
		//判断配置
		KaiFuYaoShenConfig config = KaiFuYaoShenConfigExportService.getInstance().loadByKeyId(subId,configId);
		if(config== null){
			return AppErrorCode.CONFIG_ERROR;
		}
		//玩家妖神
		RoleYaoshen yaoshen = yaoshenExportServic.getRoleYaoshen(userRoleId);
		...
		//发放奖励
		roleBagExportService.putGoodsVoAndNumberAttr(config.getJiangitem(), userRoleId, GoodsSource.GOODS_GET_CBPM, LogPrintHandle.GET_RFB_YAOSHENPAIMING, LogPrintHandle.GBZ_RFB_YAOSHEN, true);
		...
		GamePublishEvent.publishEvent(new KaiFuPaiMingLingQuLogEvent(userRoleId, loginRole.getName(), receiveItems, LogPrintHandle.KAIFU_ACTITY_LQ_YS));
	...
	public Object[] getRefbInfo(Long userRoleId, Integer subId){
		...
		Object[] one = rankExportService.getKuiShouInfoByType(GameConstants.YS_TYPE);
		int myMingci = rankExportService.getMyOrderByType(userRoleId, GameConstants.YS_TYPE);
