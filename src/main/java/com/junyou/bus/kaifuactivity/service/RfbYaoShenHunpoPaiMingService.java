...
import com.junyou.bus.kaifuactivity.configure.export.KaiFuYaoShenHunpoConfig;
import com.junyou.bus.kaifuactivity.configure.export.KaiFuYaoShenHunpoConfigExportService;
import com.junyou.bus.kaifuactivity.configure.export.KaiFuYaoShenHunpoGroupConfig;
 ...
 */
@Service
public class RfbYaoShenHunpoPaiMingService { 
	...
	@Autowired
	private YaoshenHunpoExportService yaoshenHunpoExportServic;
	/**妖神魂魄排行相关 **/
	public void start(List<IYaoShenHunpoRankVo> list) {
		Map<Integer,KaiFuYaoShenHunpoGroupConfig> vo = KaiFuYaoShenHunpoConfigExportService.getInstance().getAllConfig();
	...
	public void zhanLiPaiXing(List<IYaoShenHunpoRankVo> userList,int subId) {
		//判断是否已经发过邮件
		...
		KaiFuYaoShenHunpoGroupConfig config = KaiFuYaoShenHunpoConfigExportService.getInstance().loadByMap(subId);
		if(config == null){
			return;
		}
		//处理数据
		Map<Integer, KaiFuYaoShenHunpoConfig> configMap = config.getConfigMap();
		...
		try {
			...
			GamePublishEvent.publishEvent(new PaiMingLogEvent(LogPrintHandle.PAIMING_YAOSHEN_HUNPO_PAIMING,consumeItemArray));
		} catch (Exception e) {
			ChuanQiLog.error(""+e);
		}
		...
		for (int i = 21; i <= 40; i++) {
			...
			for (int j = start; j <= end; j++) {
				IYaoShenHunpoRankVo vo =  userList.get(j-1);
				long userRoleId = vo.getUserRoleId();
				String title = EmailUtil.getCodeEmail(AppErrorCode.KAIFU_PAIMING_EMAIL_TITLE);
				String content = EmailUtil.getCodeEmail(AppErrorCode.KAIFU_YAOSHEN_HUNPO_PAIMING,j+"");
				emailExportService.sendEmailToOne(userRoleId,title, content,GameConstants.EMAIL_TYPE_SINGLE, zhanliConfig.getEmailItem());
			
				//日志
				GamePublishEvent.publishEvent(new KaiFuPaiMingHDLogEvent(userRoleId, vo.getName(), j, zhanliConfig.getEmailItem(), LogPrintHandle.YAOSHEN_HUNPO));
			}
	 ...
	 * @param receiveItems
	 */
	public static void parseJSONArray(List<IYaoShenHunpoRankVo> userList,JSONArray receiveItems) {
		if(userList == null || userList.size() == 0){
			return;
		}
		for (int i = 0; i < userList.size();i++) {
			IYaoShenHunpoRankVo vo =  userList.get(i);
	...
	public Object[] lingQuJiangLi(long userRoleId,Integer version, Integer configId,int subId) {
		...
		//判断配置
		KaiFuYaoShenHunpoConfig config = KaiFuYaoShenHunpoConfigExportService.getInstance().loadByKeyId(subId,configId);
		if(config== null){
			return AppErrorCode.CONFIG_ERROR;
		}
		//玩家妖神
		RoleYaoshenHunpo yaoshen = yaoshenHunpoExportServic.getRoleYaoshenHunpo(userRoleId);
		if(yaoshen == null || yaoshen.getJieLevel() < config.getYaoshenLevel()){
		...
		//发放奖励
		roleBagExportService.putGoodsVoAndNumberAttr(config.getJiangitem(), userRoleId, GoodsSource.GOODS_GET_CBPM, LogPrintHandle.GET_RFB_YAOSHENHUNPOPAIMING, LogPrintHandle.GBZ_RFB_YAOSHENHUNPO, true);
		...
		GamePublishEvent.publishEvent(new KaiFuPaiMingLingQuLogEvent(userRoleId, loginRole.getName(), receiveItems, LogPrintHandle.KAIFU_ACTITY_LQ_YSHP));
	...
	public Object[] getRefbInfo(Long userRoleId, Integer subId){
		...
		Object[] one = rankExportService.getKuiShouInfoByType(GameConstants.YS_HUNPO_TYPE);
		int myMingci = rankExportService.getMyOrderByType(userRoleId, GameConstants.YS_HUNPO_TYPE);
