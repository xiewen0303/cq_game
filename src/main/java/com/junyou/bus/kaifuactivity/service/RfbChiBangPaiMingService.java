...
import com.junyou.bus.kaifuactivity.configure.export.KaiFuChiBangConfig;
import com.junyou.bus.kaifuactivity.configure.export.KaiFuChiBangConfigExportService;
import com.junyou.bus.kaifuactivity.configure.export.KaiFuChiBangGroupConfig;
 ...
 */
@Service
public class RfbChiBangPaiMingService extends AbstractActivityService {

	@Override
	public boolean getChildFlag(long userRoleId,int subId) {
		
		KaiFuChiBangGroupConfig configs = KaiFuChiBangConfigExportService.getInstance().loadByMap(subId);
		if(configs == null){
			return false;
		}
		
		//处理数据
		Map<Integer, KaiFuChiBangConfig> configMap = configs.getConfigMap();
		
		//如果这两个同时存在,则不可领取奖励
		KaiFuChiBangConfig zhanliConfig =  configMap.get(1);//ID为1规定为阶段邮件奖励
		KaiFuChiBangConfig paihengConfig =  configMap.get(21);//判断是否有排行配置
		if(paihengConfig != null && zhanliConfig != null){
			return false;
		}
		
		ChiBangInfo info = chiBangExportService.getChiBangInfo(userRoleId);
		if(info == null){
			return false;
		}
		
		for (KaiFuChiBangConfig config : configMap.values()) {
			if(config.getId() < 0){
				continue;
			}
			
			ChiBangJiChuConfig jcConfig = chiBangJiChuConfigExportService.loadById(info.getChibangLevel());
	...
	/**翅膀排行相关 **/
	public void start(List<IChiBangRankVo> list) {
		Map<Integer,KaiFuChiBangGroupConfig> vo = KaiFuChiBangConfigExportService.getInstance().getAllConfig();
	...
	public void zhanLiPaiXing(List<IChiBangRankVo> userList,int subId) {
		//判断是否已经发过邮件
		...
		KaiFuChiBangGroupConfig config = KaiFuChiBangConfigExportService.getInstance().loadByMap(subId);
		if(config == null){
			return;
		}
		//处理数据
		Map<Integer, KaiFuChiBangConfig> configMap = config.getConfigMap();
		if(userList != null && userList.size() > 0){
			//记录排名日志
			try {
				JSONArray consumeItemArray = new JSONArray(); 
				parseJSONArray(userList,consumeItemArray);
				GamePublishEvent.publishEvent(new PaiMingLogEvent(LogPrintHandle.PAIMING_ACTITY_CHIBANG,consumeItemArray));
			} catch (Exception e) {
				ChuanQiLog.error(""+e);
			}
			
			for (int i = 21; i <= 40; i++) {
				KaiFuChiBangConfig zhanliConfig =  configMap.get(i);
				...
				for (int j = start; j <= end; j++) {
					IChiBangRankVo vo =  userList.get(j-1);
					long userRoleId = vo.getUserRoleId();
					String title = EmailUtil.getCodeEmail(AppErrorCode.KAIFU_PAIMING_EMAIL_TITLE);
					String content = EmailUtil.getCodeEmail(AppErrorCode.KAIFU_CHIBANG_PAIMING,j+"");
					emailExportService.sendEmailToOne(userRoleId, title,content,GameConstants.EMAIL_TYPE_SINGLE, zhanliConfig.getEmailItem());
				
					//日志
					GamePublishEvent.publishEvent(new KaiFuPaiMingHDLogEvent(userRoleId, vo.getName(), j, zhanliConfig.getEmailItem(), LogPrintHandle.KAIFU_ACTITY_CHIBANG));
				}
		...
		if(paihengConfig != null && zhanliConfig != null){
			...
			//查询所有大于等于该等级的角色ID
			List<Long> roleIds = chiBangInfoDao.getRoleIdByLevel(jie);
			String title = EmailUtil.getCodeEmail(AppErrorCode.KAIFU_JIEDUAN_TITIE);
			String content = EmailUtil.getCodeEmail(AppErrorCode.KAIFU_CHIBANG_JIEDUAN,jie+"");
	 ...
	 * @param receiveItems
	 */
	public static void parseJSONArray(List<IChiBangRankVo> userList,JSONArray receiveItems) {
		if(userList == null || userList.size() == 0){
			return;
		}
		for (int i = 0; i < userList.size();i++) {
			IChiBangRankVo vo =  userList.get(i);
	...
	public Object[] lingQuJiangLi(long userRoleId,Integer version, Integer configId,int subId) {
		...
		//判断配置
		KaiFuChiBangConfig config = KaiFuChiBangConfigExportService.getInstance().loadByKeyId(subId,configId);
		if(config== null){
		...
		if(chibangConfig == null || chibangConfig.getLevel() < config.getChibangLevel()){
			return AppErrorCode.KAIFU_TIAOJIAN_ERROR;
		}
		...
		GamePublishEvent.publishEvent(new KaiFuPaiMingLingQuLogEvent(userRoleId, loginRole.getName(), receiveItems, LogPrintHandle.KAIFU_ACTITY_LQ_CHIBANG));
	...
	public Object[] getRefbInfo(Long userRoleId, Integer subId){
		...
		Object[] one = rankExportService.getKuiShouInfoByType(GameConstants.CB_TYPE);
		int myMingci = rankExportService.getMyOrderByType(userRoleId, GameConstants.CB_TYPE);
